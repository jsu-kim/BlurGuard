...
def generate_and_process_masks(model, image, device, points_per_side, pred_iou_thresh=0.86, stability_score_thresh=0.92, 
    ...
    """
    Generates masks for the image using the model and processes them to handle overlaps and unused areas.

    :param model: The segmentation model.
    :param image: The input image as a NumPy array.
    :param device: Device to perform computation ('cuda' or 'cpu').
    :param points_per_side: Number of points per side to use in mask generation.
    :param pred_iou_thresh: IOU threshold for prediction.
    :param stability_score_thresh: Threshold for stability score.
    :param crop_n_layers: Number of layers for cropping.
    :param crop_n_points_downscale_factor: Downscale factor for points during cropping.
    :param min_mask_region_area: Minimum area for a mask region.
    :return: Dictionary containing processed masks.
    """
...
def save_masks_to_directory(masks_dict, directory="../out/mask"):
    """
    Saves the masks in the dictionary to the specified directory.

    :param masks_dict: Dictionary containing mask tensors.
    :param directory: Directory where masks will be saved.
    """
    if not os.path.exists(directory):
        os.makedirs(directory)

    for i, (mask_name, mask_tensor) in enumerate(masks_dict.items()):
        mask_array = mask_tensor.squeeze().cpu().numpy() * 255  # Convert to binary image (0 or 255)
...
def load_masks_to_tensor_dict(directory, device="cuda"):
    """
    Loads masks from the specified directory and converts them to a dictionary of 0/1 tensors.

    :param directory: Directory where masks are stored.
    :param device: Device to load tensors onto ('cuda' or 'cpu').
    :return: Dictionary containing mask tensors with keys as 'mask1', 'mask2', etc.
    """
    ...
    for i, mask_file in enumerate(mask_files):
        ...
        # Store in dictionary with key 'mask1', 'mask2', etc.
        masks_dict[f'mask{i+1}'] = mask_tensor
...
def cal_mse(ratio, grad,editor,X,masks_dict,noise_t,file_name):
    ...
    for i in range(num_masks):
        ...
        part_grad = zero_low_frequencies(grad, ratio[i])*mask # ratio[i] shape is [2]
        masked_grad+=part_grad 

    edit_one_step = editor.edit_list(X+masked_grad  , restep=None, t_list=[noise_t]).cuda()

    left_image = edit_one_step[:, :, :, :edit_one_step.shape[3] // 2] # adv img
    right_image = edit_one_step[:, :, :, edit_one_step.shape[3] // 2:] # denoised adv img
...
def zero_low_frequencies(x, ratio):
    ...
    mask = torch.matmul(x_mask.T, y_mask) # 512 *1 * 1* 512 -> 512 * 512
...
def ssim_(p1, p2):
    i1_pil = Image.open(p1).convert('RGB').resize((512, 512), Image.BICUBIC)
    i2_pil = Image.open(p2).convert('RGB').resize((512, 512), Image.BICUBIC)
    
    i1 = cv2.cvtColor(np.array(i1_pil), cv2.COLOR_RGB2BGR)
    i2 = cv2.cvtColor(np.array(i2_pil), cv2.COLOR_RGB2BGR)
...
def psnr_(a, b):
    original_pil = Image.open(a).convert('RGB').resize((512, 512), Image.BICUBIC)
    compressed_pil = Image.open(b).convert('RGB').resize((512, 512), Image.BICUBIC)
    
    original = cv2.cvtColor(np.array(original_pil), cv2.COLOR_RGB2BGR)
    compressed = cv2.cvtColor(np.array(compressed_pil), cv2.COLOR_RGB2BGR)
